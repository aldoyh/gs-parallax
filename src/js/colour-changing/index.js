...
import labToRgb from "./lab-to-rgb.js";
  ...
  Object.keys(start.colours).forEach((key) => {
    ...
    for (let i = 0; i < startColour.length; i++) {
      currColour[i] = Math.round(
        lerp(startColour[i], endColour[i], progressCurr)
      );
    }
...
function activateSun() {
  const sunWrap = document.querySelector(".landscape__sun");
  if (sunWrap) {
...
function changeTheme(themeSlug) {
  ...
  const currentTheme = document.querySelector(`[data-theme="${themeSlug}"]`);
